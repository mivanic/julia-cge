...
loop(reg,
         alpha(reg,"com1")= ord(reg)/card(reg) * 0.6
loop(comm,
         alpha(reg,comm) = (1-alpha(reg,"com1"))/(card(com)-1)
););

loop(reg,
         alpha2(reg,"fac1")= ord(reg)/card(reg) * 0.4
loop(facc,
         alpha2(reg,facc) = (1-alpha2(reg,"fac1"))/(card(com)-1)
););

loop(reg,
loop(com,
         gamma(reg,com) = ord(reg)/card(reg)*ord(com)/card(com) * 0.15
););
...
loop(fac,
loop(reg,
                           qet.fx[fac,reg] =  ord(fac)/2+ ord(reg);
);
) ;
$ontext
qet.fx["labor","usa"] =  4;
...
display alpha, gamma, alpha2, qp.l;